...
//		imageGrid.requestFocus();
	...
	private int cellPos(int x, int y) {
		//System.out.println("cell Poss"+y);
		return x * imageGrid.getGridColumnCount() + y;
	}
	...
	protected void mouseClicked(MouseEvent mouseEvent) {
		counterClicks++;
		int clicks = counterClicks-mouseEvent.getClickCount();
		Node child = mouseEvent.getPickResult().getIntersectedNode();
		Boolean result = applyCell((x, y) -> mouseCllicked(x, y, clicks), child);
		if (result != null) {
			mouseEvent.consume();
		}
	}
